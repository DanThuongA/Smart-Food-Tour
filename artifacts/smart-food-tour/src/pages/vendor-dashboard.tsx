...
import {
  ...
  Settings, LogOut, ChevronRight, Star, Eye, Volume2,
  Users, TrendingUp, Bell, Plus, Edit2, Trash2, CheckCircle, Clock, Menu, X
} from "lucide-react";
...
type Tab = "overview" | "venues" | "audio" | "stats" | "messages" | "settings";
      ...
      <div className="p-3 flex-1">
        ...
        {NAV_ITEMS.map(({ id, label, icon: Icon }) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl mb-1 transition-colors text-sm font-medium ${tab === id ? "bg-orange-500 text-white" : "text-gray-600 hover:bg-orange-50"}`}
          >
            <Icon size={16} /> {label}
          </button>
        ))}
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6">
          ...
          {tab === "overview" && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard label="Lượt tiếp cận tuần" value={stats?.weeklyVisits} icon={TrendingUp} color="bg-orange-500" />
                <StatCard label="Lượt nghe audio" value={stats?.audioPlays} icon={Volume2} color="bg-blue-500" />
                <StatCard label="Đang gần quán" value={stats?.nearbyNow} icon={Users} color="bg-green-500" />
                <StatCard label="Quán của tôi" value={MOCK_VENUES.length} icon={Store} color="bg-purple-500" />
              </div>

              {/* Weekly Chart */}
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                <h2 className="font-semibold text-gray-700 mb-4">Traffic 7 ngày</h2>
                <div className="flex items-end gap-2 h-32">
                  {(stats?.dailyTraffic || [
                    { day: "T2", visits: 120 }, { day: "T3", visits: 95 }, { day: "T4", visits: 140 },
                    { day: "T5", visits: 110 }, { day: "T6", visits: 185 }, { day: "T7", visits: 220 }, { day: "CN", visits: 160 }
                  ]).map((d: any) => {
                    const max = 250;
                    ...
                    return (
                      <div key={d.day} className="flex-1 flex flex-col items-center gap-1">
                        ...
                        <div className="w-full bg-orange-100 rounded-t-md" style={{ height: `${h}%` }}>
                          <div className="w-full h-full bg-orange-400 rounded-t-md hover:bg-orange-500 transition-colors" />
                        </div>
              ...
              {MOCK_VENUES.map((v) => (
                ...
                >
                  ...
                  <div className="flex gap-4 mt-3 pt-3 border-t border-gray-50">
                    <div className="flex items-center gap-1 text-xs text-gray-500"><Eye size={13} /> {v.visits} lượt xem</div>
          ...
          {tab === "audio" && (
            <div className="space-y-4">
              ...
              {MOCK_VENUES.filter(v => v.status === "active").map((v) => (
                <div key={v.id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={item.lang} className="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
          ...
          {tab === "stats" && (
            <div className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                  <h3 className="font-semibold text-gray-700 mb-4">Ngôn ngữ du khách</h3>
                  {[
                    { lang: "🇻🇳 Tiếng Việt", pct: 42 },
                    { lang: "🇬🇧 English", pct: 28 },
                    { lang: "🇨🇳 中文", pct: 15 },
                    { lang: "🇯🇵 日本語", pct: 9 },
                    { lang: "🇰🇷 한국어", pct: 6 },
                  ].map((item) => (
                    <div key={item.lang} className="flex items-center gap-3 mb-2">
                      <span className="text-xs text-gray-600 w-28">{item.lang}</span>
                      <div className="flex-1 h-2 bg-gray-100 rounded-full">
                        <div className="h-full bg-orange-400 rounded-full transition-all" style={{ width: `${item.pct}%` }} />
                      </div>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                  <h3 className="font-semibold text-gray-700 mb-4">Đánh giá tổng hợp</h3>
                  <div className="text-center py-4">
                    <p className="text-5xl font-bold text-orange-500">4.8</p>
                    <div className="flex justify-center gap-1 mt-2">
                      {[1,2,3,4,5].map(i => <Star key={i} size={20} className={i <= 4 ? "fill-orange-400 text-orange-400" : "text-gray-200 fill-gray-200"} />)}
                    </div>
                    <p className="text-sm text-gray-500 mt-2">Từ 1,243 đánh giá</p>
                  </div>
                    ...
                    {[5,4,3,2,1].map(s => (
                      <div key={s} className="flex items-center gap-2">
                        ...
                        <div className="flex-1 h-1.5 bg-gray-100 rounded-full">
                          <div className="h-full bg-yellow-400 rounded-full" style={{ width: `${[78, 14, 5, 2, 1][5-s]}%` }} />
                        </div>
                        <span className="text-xs text-gray-400">{[78,14,5,2,1][5-s]}%</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="space-y-3">
              {MOCK_MSGS.map((m) => (
                <motion.div
                  ...
                  className={`bg-white rounded-2xl p-4 shadow-sm border cursor-pointer hover:border-orange-200 transition-colors ${!m.read ? "border-orange-300" : "border-gray-100"}`}
                >
          ...
          {/* === SETTINGS === */}
          {tab === "settings" && (
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 max-w-lg space-y-5">
